...
data class ComponentDef(
    ...
    fun layoutName() =
        name.map { if (it.isUpperCase()) "_${it.lowercase()}" else it }
            .joinToString(separator = "").substring(1)

    fun binding(viewModuleAndroidPackage: String) = ClassName("$viewModuleAndroidPackage.view.databinding", name.suffix("Binding"))
    ...
    fun toFillVCparams() =
        (
            subComponents.toFillParams(init = { "${name.toVM()}.view" }) + fixProperties.toFillParams() +
                mutableProperties.map { it.initial() }
                    .toFillParams()
    ...
    val states =
        modelClass?.ownProperties()?.filter {
...
data class PropertyDef(
    ...
    fun asParam(withDefaultNullIfNullable: Boolean = false): ParameterSpec = ParameterSpec.builder(name, type).build()
...
fun MutableSet<KClass<out SKComponentVC>>.addLinkedComponents(
    ...
    add(aComponentClass)
    val subComponents =
        aComponentClass.ownProperties().map { it.returnType }.filter {
...
fun KClass<out SKComponentVC>.def(): ComponentDef {
    ...
    val stateProperties = ownProperties - subComponentsProperties.toSet()
    ...
    return ComponentDef(
        ...
        subComponents =
            subComponentsProperties.map {
                if (it is KMutableProperty) {
                    throw IllegalStateException(
                        "SubComponent ${it.name} of ${this.packageName()}.${this.simpleName} is Mutable, it is not allowed !!!",
                    )
                }
                PropertyDef(
                    it.name,
        ...
        fixProperties =
            stateProperties.filter { (it !is KMutableProperty) }.map {
                PropertyDef(name = it.name, type = it.returnType.asCleanTypeName())
            },
        mutableProperties =
...
fun KClass<*>.packageName() : String = this.java.`package`.name
