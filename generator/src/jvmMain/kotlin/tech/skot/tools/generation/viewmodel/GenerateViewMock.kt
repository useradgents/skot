...
fun Generator.generateViewMock() {
    components.forEach {
        if (!it.viewMock().existsJvmTestInModule(modules.viewmodel)) {
            it.viewMock().fileClassBuilder {
                ...
                it.ownFunctions.forEach {
                    ...
                    addProperty(
                        PropertySpec.builder(
                            callsProperty,
                            ClassName("kotlin.collections", listOf("MutableList")).parameterizedBy(
                                if (withParams) {
...
fun TypeName.toViewMock() = (this as ClassName).let { ClassName(it.packageName, it.simpleName.toViewMock()) }

fun String.toViewMock() =
    when {
        endsWith("VC") -> {
