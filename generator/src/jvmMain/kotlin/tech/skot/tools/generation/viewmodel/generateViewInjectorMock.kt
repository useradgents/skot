...
fun Generator.generateViewInjectorMock() {
    FileSpec.builder(viewInjectorMock.packageName, viewInjectorMock.simpleName)
        .addType(
            TypeSpec.classBuilder(viewInjectorMock.simpleName)
...
fun ComponentDef.asViewMockInjection(generator: Generator) =
    FunSpec.builder(name.decapitalizeAsciiOnly())
        ...
        .returns(vc)
        .addCode(
            "return ${viewMock().simpleName}(${(subComponents.map { "${it.name} as ${it.type.toViewMock().simpleName()}" } + (fixProperties.map { it.name } + mutableProperties.map { it.initial().name })).joinToString(
                separator = ",",
            )})",
        )
        .build()
