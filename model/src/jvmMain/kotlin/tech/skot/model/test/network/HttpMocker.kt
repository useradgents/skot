...
class HttpMocker() {
    ...
    fun HttpMocker.setResponseWithException(
        encodedPath: String,
        throwable: Throwable,
    ) {
        responsesForUrl[encodedPath] =
    ...
    fun findCall(encodedPath: String): HttpRequestData? = calls.find { it.pathEqualTo(encodedPath) }

    val nbCalls: Int
        get() = calls.size

    suspend fun assertCalled(
        encodedPath: String,
        body: String? = null,
        rule: String = "",
    ) {
        ...
        body?.let {
            assertEquals(
                ...
                expected = it,
                actual =
                    calls.find { it.pathEqualTo(encodedPath) }?.body?.toByteReadPacket()
                        ?.readText(),
            )
        }
    }
    ...
    fun request(
        ...
        val response =
            mockHttp.nextResponse ?: mockHttp.responsesForUrl[requestPath]
                ?: pathPrefix?.let {
